...
#define ARRrand(A,i,j,k) A[(i)*((long)n3iso_bigger)*((long)n2iso_bigger)+(j)*((long)n3iso_bigger)+(k)]
...
int pow3iso(double dx, double hurst, double l1, double seed, double n1iso_bigger, double n2iso_bigger,
	...
	if (fp != NULL) {
        ...
        fprintf(stdout, "debug: randarray = %f, %f\n", ARRrand(randarray, 0, 1, 0), ARRrand(randarray2, 0, 1, 0));
	...
	for (j = 0; j < n3iso_bigger; j++) {
        ...
        //fprintf(stdout, "debug: %lf + %lfi\n", ARRs3d(result, 0, 0, 0)[0], ARRs3d(result, 0, 0, 0)[1]);
        /*char fname[100];
        sprintf(fname, "result.bin");
        FILE *fid;
        fid=fopen(fname, "w");
        fwrite(result, sizeof(double), 2*totalsize, fid);
        fclose(fid);*/
	...
	for (i = 0; i < n1iso_bigger; i++) {
        ...
        //fprintf(stdout, "Sum of result: %20.15le + %20.15lei\n", xbar[0], xbar[1]);
	...
	for (i = 0; i < n1iso_bigger; i++) {
        ...
        //fprintf(stdout, "distBeforeDiv=%lf\n", distBeforeDiv);
...
void fftfreq(int n, double dh, double *freq){
   int k;
   float df;

   df = 1.f/(n * dh);

   for (k=0; k<n/2; k++) freq[k] = k * df;
   for (k=n/2; k<n; k++) freq[k] = -(n-k) * df;

                /*if (j < (n3iso_bigger < 2)) wnj = j * dfh;
                else wnj=-(n3iso_bigger - j) * dfh;
                        if (i < (n1iso_bigger < 2)) wni = i * df;
                        else wni=-(n1iso_bigger - i) * df;
                                if (k < (n2iso_bigger < 2)) wnk = k * dfh;*/
}

//Added by Daniel, to remove wavenumbers not resolvable by coarse grid in DM computations
void downsample_pow3(double n1iso_bigger, double n2iso_bigger, double n3iso_bigger, fftw_complex *insim, fftw_complex *outsim,
  ...
  size_t totalsize;
	...
	wncut = 1.f/(2.f*dwnsmpfct); /* nyquist wavenumber of coarse grid (dimensionless) */

	totalsize = (long)n1iso_bigger * (long)n2iso_bigger * (long)n3iso_bigger;

        /*wavenumbers of input (dimensionless*/
        wnj=(double*) calloc(n3iso_bigger, sizeof(double));
        wni=(double*) calloc(n1iso_bigger, sizeof(double));
        wnk=(double*) calloc(n2iso_bigger, sizeof(double));

        fftfreq(n3iso_bigger, st23, wnj);
        fftfreq(n1iso_bigger, 1.f, wni);
        fftfreq(n2iso_bigger, st23, wnk);

        //for (i=0; i<n1iso_bigger; i++) fprintf(stdout, "wni[%d]=%f\n", i, wni[i]);
        result = (fftw_complex*) calloc(totalsize, sizeof(fftw_complex));

	p = fftw_plan_dft_3d((int)n1iso_bigger, (int)n2iso_bigger, (int)n3iso_bigger, insim, result, FFTW_FORWARD, FFTW_ESTIMATE);
	...
	for (j = 0; j < n3iso_bigger; j++) {
		for (i = 0; i < n1iso_bigger; i++) {
			for (k = 0; k < n2iso_bigger; k++) {
                                if (fabs(wnj[j]) > wncut || fabs(wni[i]) > wncut || fabs(wnk[k]) > wncut){
				      ARRs3d(result, i, k, j)[0] = 0.f;
				      ARRs3d(result, i, k, j)[1] = 0.f;
                                }
                        }
                }
        }

	p = fftw_plan_dft_3d((int)n1iso_bigger, (int)n2iso_bigger, (int)n3iso_bigger, result, outsim, FFTW_BACKWARD, FFTW_ESTIMATE);
	fftw_execute(p);
	fftw_destroy_plan(p);
	...
	for (j = 0; j < n3iso_bigger; j++) {
		for (i = 0; i < n1iso_bigger; i++) {
			for (k = 0; k < n2iso_bigger; k++) {
				ARRs3d(outsim, i, k, j)[0] = ARRs3d(outsim, i, k, j)[0] / totalsize;
				ARRs3d(outsim, i, k, j)[1] = ARRs3d(outsim, i, k, j)[1] / totalsize;
			}
		}
	}
